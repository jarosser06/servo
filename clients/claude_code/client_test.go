...
func TestClient_AddServer(t *testing.T) {
	client := NewWithExecutor(&MockCommandExecutor{})
	err := client.AddServer("local", "test-server", "test-command", []string{"--arg1"})
	if err != nil {
...
func TestClient_ListServers(t *testing.T) {
	...
	// Should return the mock servers
	if len(servers) != 2 {
		t.Errorf("expected 2 servers, got %d", len(servers))
	}
}

func TestClient_EnableServer(t *testing.T) {
	client := NewWithExecutor(&MockCommandExecutor{})
	err := client.EnableServer("local", "test-server")
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
}

func TestClient_DisableServer(t *testing.T) {
	client := NewWithExecutor(&MockCommandExecutor{})
	err := client.DisableServer("local", "test-server")
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
}
